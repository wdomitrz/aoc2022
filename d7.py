...
d = list(
    map(
        lambda x: tuple(map(lambda y: y.split(" "), x.splitlines())),
        """
$ cd /
$ ls
dir brhvclj
...
p1_res = sum(x for x in dir_sizes.values() if x <= 100000)
print(p1_res)

current_free_space = 70000000 - dir_sizes[("/",)]
needed_space = 30000000 - current_free_space

p2_res = min(x for x in dir_sizes.values() if x >= needed_space)
print(p2_res)
