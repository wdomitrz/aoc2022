# %%
d_raw = """\
                                                  ......#....#.#........................................#.......##...........#.............#......#...
                                                  ..............#..#........#......##........................#......#....#........................#...
                                                  .....................#.........#...#.#........#..............................#....#........#..#.....
                                                  .........#.........##................................#.....#.........#...........#...........#......
                                                  ..........#.............#.....##.......#..#..........##......#.#...........#............##..........
                                                  ......#.............##.........#.....#....#...............................................#.......#.
                                                  ..#......#......#.........#..#.........#....#...................#...........#...#.....##.#..#...#..#
                                                  #.#.............#.............#.#......#.....#.....#.##.##.....#...#..#...............#.............
                                                  .#....#...##....#...................#..#............................................#.........#.....
                                                  ....#..##....#..#....#..............#..#...#...#................#...........#..#..#............##...
                                                  ..#.#....................................................................#.....##....##.....#..#....
                                                  .......#...#......#...##........#...#......................#.#.#..............#...#..#.....#........
                                                  .....#.#..#........#..............##......#........##...............#..................#.....#......
                                                  .......#...............#...#...................................#............#.......................
                                                  ......#..##.....#.......#....#..................................................#.#.......#.........
                                                  ................#..#..#....#.#.....................#................................................
                                                  .................#.#........#.....##...#...#.#..#....#....#.............#...#..................#..#.
                                                  .#...................#......#..#...............#.............#.........................#...#.....#..
                                                  ...#.#...................#...#.........................#.......##.......................#.........#.
                                                  .#.............................#........##.........#.#.....##.....................#....#...........#
                                                  ..........#.....#..............................#................##.....................#.......#....
                                                  .....................................#....##..........#...............#.......##.........#.#........
                                                  ....#.###............#....#..........#..........#.......................#...............#....#.#....
                                                  .#...#...#...............#.........#........................#.................#..................#.#
                                                  .................#......#.....................#........#......#.....................#....#.##.......
                                                  ##..#....................#.....................#....#....#.....#.............................#.....#
                                                  #...#.......................................................##.#...#.........#..#...................
                                                  ............#......#.....#........#..#.#......#.#.............#....#......#.#...............#.......
                                                  ..................................................#.#..........##.........##......#.#.#.........##..
                                                  ..#..........................#.......#..#..#.......#.................###.......#...........#..#...##
                                                  ..#.....#..#...#.....#.....#...#..#.................................#.....#.........#.............#.
                                                  ............#.#..#..##..#.#.##....#...............#.......#........#............#........#..........
                                                  .#...........#...........................#............#.....#.#.........#.....#.......###...#.......
                                                  ...#.......................#.........................#...##........#.....................#.......#..
                                                  ...........#..#.............................#.................#..##...#.............................
                                                  ..................#.#...#..#....#....#..........#................#..#.........#......#.......#......
                                                  ...#...#....#.#................#.............#.............................#..#.....................
                                                  ....................#.........#..#.#...#.#.##...#...#..............#.......#.........#.............#
                                                  ...#.....#.............##.....#....#.#.......#......................................................
                                                  ..........#..#...#.......................#.................##.....#........#...#......#.......#.....
                                                  ........#......#...............#.#.....#...........................#.....................#.......#..
                                                  ..................................#...........#......#...........#.............#....###.............
                                                  ................#.......................#.#......#.##..........#......#............#................
                                                  .........................................#..........................................##...........#..
                                                  ...#.......#....##.#....#...............#..#...#.............#...#....#.....#.#...........#.........
                                                  #........#...........#...........#.....................#........#..................#.....#......#...
                                                  ........#.....##..#.........................#....#..........#.##..#.......#.#.................#...#.
                                                  ..........................#.............................#........#............#.............#....#..
                                                  .............................#..................................................#...................
                                                  ................#.............#..#..........##......#......#.........#...........##............##...
                                                  #...#.......#.............#............#..........
                                                  #................#...#....#..................#....
                                                  .................#.....#.#....#......#...#........
                                                  ..................................#...............
                                                  .....#...............................#............
                                                  ...............#............##..........#.......#.
                                                  #............#.....................###.#.........#
                                                  .#...............#...........#....#......#........
                                                  .#.............###.#................#........#....
                                                  ....##............#..........#.#.........#.#.#....
                                                  ..................#..#..............#......#......
                                                  ....#.............................#...............
                                                  .#.#..........#...........#......##...............
                                                  ..........#......#....#...#.#....#..#.............
                                                  ................#...............#.................
                                                  ..........#..#...#..............#...............#.
                                                  ...#..............##..............#...............
                                                  ..#...#.#....#.........#.........#.......#........
                                                  .................#..............#.........#.....#.
                                                  .......#........#......#............#......#......
                                                  ..........#..............#....#..#............#...
                                                  .......#............#.............#...............
                                                  .....#.#..........#.#..........#..................
                                                  #...#...................#......#......#.....#.....
                                                  ........#........##.#.......##................#...
                                                  ..................#.........................##....
                                                  ..#................#..............#.#....#.#.#...#
                                                  ........#..........................#..........#...
                                                  ..#.......#.#............#..........#.....#.......
                                                  ....#....#...............................#........
                                                  ..#............................................#..
                                                  ....#.......#..............##.......#......##.....
                                                  .#..........#......#....#.#........#..............
                                                  ................#.#.......##.............###.....#
                                                  ...............#......##.#...#................#...
                                                  ....#.......#....##...#..#..........#.........#...
                                                  .....#.........#...#...........#........#.........
                                                  .......#...............#..................#.......
                                                  ...........#.....#.........#........#....#.......#
                                                  ..#.....................................#...#.....
                                                  ..................#..............#....#.#.........
                                                  ........................#.......#.#......#........
                                                  ...#......#..#..........#..#..#........#..........
                                                  .....#.......#.........#.............#.......#....
                                                  ......#......#...............#.....#.........#....
                                                  .#.........................#.....#.............#..
                                                  ...#......#.#.....##...#.#....#.#...............#.
                                                  .............#........#.#.#.....#.................
                                                  ...#.........#.........#....#.................##..
                                                  .......#.......##...#....#..........#.#...........
..#......#....................#.........#........#.....................#............#.....#.........
.##...........##...................#.#..................................#...........................
....#............#..........#..........................#...#......................#....##...........
.#...........#........................#..............................#..#...................#......#
#................#.#........#...........................#....................#...........#..........
#.....................#.....#.......##....#......#..##...#...........#...................#..........
.........#..#......#....#......#....#.......#.....................................................##
..#...#......##.#..................#.......#.............#....#....#.........#..............#.....#.
...........#...#...#.#........#..................##...........#.....#...............................
.............................#....##..#..#..........#.............................#.................
...#.................#...#...#...#.................................................#..............##
......#........#........#.........##....#....#.....#.......#.............#...##.##.#................
..#.......................#....................#..#...#.................#......#..........#......#..
.........................#....#..##..............#.............#.#...#....#.......#...#....#.....#..
....#................#.#...........................#....#..#.......##.#.....#................#.#....
..#.......................#.....#...#..#..#...#...#.............................#............#......
#..............................................#.......................................##........#.#
..............#.#............##..........#.........................#..................#.#...........
.....#......................#...........#.....#.................#................#........#...#..#.#
......#.#....#......#.#..................#.#................#.#.#...........#............#.....#....
.#......#......##..............#.........#.#.#.#..................#...#..........#..................
...#...........#.............#....#.#...#....#.......................#.#.....#.....#..............#.
...#...#.#.........#...#............#....#.........#.....#......#..#...........#.....#......#.......
.#.....#..........#..............#..#........#..............#...#.............#.#.......#..#........
.....#........................#.....#......#...............##..#.#...#...#.#............#...........
.....#......#.........#....#.......##.......#.##..................#...............#......#........#.
.#..#..#..#.................................#......#.#............#..#....#...................#.....
.#..#...........................#....#............................................................#.
......#..............................#..#.......#.........##..........##...#........................
............#...............#..#..#..........#......................#...#.#............#............
...##..................#.....#.........#......#...........#.......#.....#.......................#...
..#..#..#......#......#.....#.......#.................##..#.........................................
............#...##...#..........#.................................##....#........##........#........
...##...#....#....#........................#.............##.........................##.......#......
.............##..................#...................##.......#........##.............#.#..........#
.......#.....#...##..............#..........................#..........#..........#..#.............#
............#...#........#........#...............#.....#.....#.......#..............#.....#......#.
...........#.....#..##.................#......#........#.#.....................#....................
.....#......#.........#..........#..#......................................................##.......
..........#.....#......#.#.........#.....#..................##.............................#........
##......#...#..........#...............#.............#........#..................#........#........#
..#.........#...........#.......#...........##...................#.................#.#.....#...#....
...#.........#..................................#.....#..#............#.............#.#.............
....#...........#..........#..............#..###........#..#.........#....................#.#.......
..#..........#..#...............................#...#.................#............#............#.#.
.....................#.............#....#..#....#......................#..#......#..................
........#.#.............#....#...#.#.....#..........#................#.......#........#....#.#.....#
##.........................##....#...............#..........#......................##...............
.........#..................................#........................#....#............#...#.#......
........#........................#..#...#..........................#................................
................#..#.......#..#...................
.............#........#...................#.......
..#....#.......................#..........#...#..#
..........#.....................#..............#.#
.........#...#...#..............#..#.........#....
...........#.....#................................
.......#..#..#.......#............#...............
.........................#..........#..#....#..#..
#...........#....#.....#....#.#..#.......#........
#.#.......#.....#........#....#...#...............
..............................................#...
#......#....................................#...#.
........##..#.....#.#......#.........#..##....#...
...............#.........#...#.......#......#..#..
..........#.#......#.....#........................
.....#.....#......................................
..#..#............#.......#.....#..#.......#...#..
...............##....#........#..#.............#.#
..............#..................#.....#......#...
#.......#.......#...#...............#.#....#......
#.........#........#..........#........#....#.....
.....###.#...........#...........##............#..
.........#..##.......#......................#.....
.....#.##......#.......#...#.............#........
##.................#.....#.............#....#...#.
........#..............#.#.........#.......#......
........#..#.#.....#.............#............#..#
.#.....#.#..#..#.##........#..........#.....##....
.....#....#.........#.............................
................#..#....#.....#....#........#..#..
#.#............#.....#.............#.........#....
...#....#..........#.#.................#.#..#...#.
........#................#...#......#......#...#.#
..#.............#....#.......#..............#.....
.....................................##.........#.
............#.#...................................
..#...........................#..........#........
...#.............#..............#..........#...#..
#......#...#...#...#...#..................#....#..
....#......#..#....#.#.#..........................
..#.......................#..............#..#.....
................#........#...#..........#.#.......
....#...#......#...................#.#.#...#....#.
............#..............#........#..#.....#....
..........#...............#.......#...............
#....................#.......#...........#........
.......#.........#..........#.#..#.#..............
..#.........##......................#...#.........
...#.................#.##............#........#...
....#......#..#.....................#.............


""".split(
    "\n\n"
)


def parse_moves(line):
    moves = []
    prev = []
    for x in line:
        if x in ["R", "L"]:
            if len(prev) != 0:
                moves.append(int("".join(prev)))
                prev = []
            moves.append(x)
        else:
            prev.append(x)
    if len(prev) != 0:
        moves.append(int("".join(prev)))
    return moves


d = d_raw[0].splitlines(), parse_moves(d_raw[1])
board = d[0]
moves = d[1]

p2_step = 4 if len(board) < 50 else 50

rows_starts = []
rows_ends = []
columns_starts = []
columns_ends = []
rows_number = len(d[0])
columns_number = max(map(len, d[0]))


for row in board:
    for j, v in enumerate(row):
        if v == " ":
            continue
        rows_starts.append(j)
        break
    for j, v in reversed(list(enumerate(row))):
        if v == " ":
            continue
        rows_ends.append(j)
        break


for i in range(columns_number):
    for j in range(rows_number):
        v = board[j][i] if len(board[j]) > i else " "
        if v == " ":
            continue
        columns_starts.append(j)
        break
    for j in reversed(range(rows_number)):
        v = board[j][i] if len(board[j]) > i else " "
        if v == " ":
            continue
        columns_ends.append(j)
        break

starting_position = (-1, -1, -1)
for i, v in enumerate(board[0]):
    if v not in [" ", "#"]:
        starting_position = (0, i, 0)
        break

dirs = ((0, 1), (1, 0), (0, -1), (-1, 0))


def perform_move(position, move) -> tuple[int, int, int]:
    if not isinstance(move, int):
        if move == "R":
            new_direction = (position[2] + 1) % len(dirs)
        elif move == "L":
            new_direction = (position[2] - 1) % len(dirs)
        else:
            raise RuntimeError(f"Unknown move {move}")
        return position[0], position[1], new_direction

    x, y, dir = position
    dx, dy = dirs[dir]
    for _ in range(move):
        nx, ny = x + dx, y + dy
        if dx != 0:
            if nx < columns_starts[y]:
                nx = columns_ends[y]
            elif columns_ends[y] < nx:
                nx = columns_starts[y]
        else:  # dy != 0:
            if ny < rows_starts[x]:
                ny = rows_ends[x]
            elif rows_ends[x] < ny:
                ny = rows_starts[x]
        if board[nx][ny] != ".":
            break
        x, y, dir = nx, ny, dir
    return x, y, dir


position = starting_position

for move in moves:
    position = perform_move(position, move)


def get_ans(position):
    x, y, dir = position
    return 1000 * (x + 1) + 4 * (y + 1) + dir


p1_res = get_ans(position)
print(p1_res)


shape = []
for i in range(0, len(board), p2_step):
    shape.append([])
    for j in range(0, len(board[i]), p2_step):
        if board[i][j] == " ":
            shape[-1].append(" ")
        else:
            shape[-1].append("X")

shape = "\n".join(["".join(row) for row in shape]) + "\n"

if (
    shape
    == """\
  X
XXX
  XX
"""
):
    changes_up = (
        # row of target square, column of target square,
        # direction, axis orientation change
        ((0, 2), 1, True),
        ((0, 2), 0, False),
        ((1, 0), 1, True),
        ((1, 2), 2, True),
    )
    changes_down = (
        ((2, 2), 3, True),
        ((2, 2), 0, True),
        ((1, 0), 3, True),
        ((1, 0), 0, True),
    )
    changes_left = (
        ((1, 1), 1, False),
        ((2, 3), 3, True),
        ((1, 1), 3, True),
    )
    changes_right = (
        ((2, 3), 2, True),
        ((2, 3), 1, True),
        ((0, 2), 2, True),
    )
elif (
    shape
    == """\
 XX
 X
XX
X
"""
):
    changes_up = (
        ((1, 1), 0, False),
        ((3, 0), 0, False),
        ((3, 0), 3, False),
    )
    changes_down = (
        ((0, 2), 1, False),
        ((3, 0), 2, False),
        ((1, 1), 2, False),
    )
    changes_left = (
        ((2, 0), 0, True),
        ((2, 0), 1, False),
        ((0, 1), 0, True),
        ((0, 1), 1, False),
    )
    changes_right = (
        ((2, 1), 2, True),
        ((0, 2), 3, False),
        ((0, 2), 2, True),
        ((2, 1), 3, False),
    )
else:
    raise RuntimeError(f"No implementation for the following shape\n{shape}")

columns_starts_2 = []
columns_ends_2 = []
rows_starts_2 = []
rows_ends_2 = []


def get_nx_ny_new_dir(pos_number, where_from):
    (new_square_x, new_square_y), new_dir, o_change = where_from[pos_number // p2_step]
    used_position = (new_square_x, new_square_y)[new_dir % 2]
    global_coord = (rows_starts, columns_starts, rows_ends, columns_ends)[new_dir][
        used_position * p2_step
    ]
    local_coord = used_position * p2_step + (
        (p2_step - 1 - (pos_number % p2_step)) if o_change else (pos_number % p2_step)
    )
    if new_dir % 2 == 0:
        return (local_coord, global_coord), new_dir
    else:
        return (global_coord, local_coord), new_dir


for col_number in range(len(columns_starts)):
    columns_starts_2.append(get_nx_ny_new_dir(col_number, changes_up))
    columns_ends_2.append(get_nx_ny_new_dir(col_number, changes_down))

for row_number in range(len(rows_starts)):
    rows_starts_2.append(get_nx_ny_new_dir(row_number, changes_left))
    rows_ends_2.append(get_nx_ny_new_dir(row_number, changes_right))


def perform_move_2(position, move) -> tuple[int, int, int]:
    if not isinstance(move, int):
        if move == "R":
            new_direction = (position[2] + 1) % len(dirs)
        elif move == "L":
            new_direction = (position[2] - 1) % len(dirs)
        else:
            raise RuntimeError(f"Unknown move {move}")
        return position[0], position[1], new_direction

    x, y, dir = position

    for _ in range(move):
        dx, dy = dirs[dir]
        nx, ny, new_dir = x + dx, y + dy, dir
        if dx != 0:
            if nx < columns_starts[y]:
                (nx, ny), new_dir = columns_starts_2[y]
            elif columns_ends[y] < nx:
                (nx, ny), new_dir = columns_ends_2[y]
        else:
            if ny < rows_starts[x]:
                (nx, ny), new_dir = rows_starts_2[x]
            elif rows_ends[x] < ny:
                (nx, ny), new_dir = rows_ends_2[x]
        if board[nx][ny] != ".":
            break
        x, y, dir = nx, ny, new_dir
    return x, y, dir


position = starting_position

for move in moves:
    position = perform_move_2(position, move)

p2_res = get_ans(position)
print(p2_res)
